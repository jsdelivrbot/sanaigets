	...
	<div class="subpage moreinfo">
		...
		<div class="row avatar">
			...
			<form id="moreinfoForm" ref="moreinfoForm">
				...
				<div class="commit" @click.stop.prevent="uploadavatar" ref="uploadavatarTs">选择完毕上传新头像</div>
			</form>
		</div>
...
export default {
	...
	methods:{
	    ...
	    uploadavatar () {
	    	this.$refs.uploadavatarTs.innerHTML = "图片上传按钮已经被点击了";
	    	let that = this;
	    	if (this.$refs.moreinfoFormfile.value == '') {
	    		return
	    	}else{
	    		that.$refs.uploadavatarTs.innerHTML = "即将上传";
				var formData = new FormData($("#moreinfoForm")[0]);
				var _useinfo = {
				...
				that.$refs.uploadavatarTs.innerHTML = "即将上传-2";
				$.ajax({
					type:"post",									
					data:formData,
					url:"http://gfire.top:8080/api/users/avatarupload",
					...
					processData: false,	//必须false才会自动加上正确的Content-Type
					async:true,
					beforeSend:function(){console.log("正在进行，即将上传 ，请稍候");that.$refs.uploadavatarTs.innerHTML="正在进行，即将上传 ，请稍候"},
					success:function (e) {
						console.log("返回结果：%O",e);
						//that.$refs.avatar.value = e.data.userinfo.avatar;
						that.$store.dispatch('updataUserInfo',e.data.userinfo);
						that.$refs.uploadavatarTs.innerHTML = "上传成功";
					},
					error:function (err) {
						that.$refs.uploadavatarTs.innerHTML = "上传失败请重试";
						console.log(err.msg);
					}
				});
			}	
