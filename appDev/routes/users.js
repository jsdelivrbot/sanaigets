	...
	connection.query(sql,function (err, result) {
	        ...
	        if(JSON.parse(JSON.stringify(result)).length!=0){
	        	...
	        	connection.query(sqlall,function (err, result) {
					...
					_avatar = req.query.avatar?req.query.avatar:"";
...
/* 修改密码接口 （http://gfire.top:8080/api/users/resetpassword?usename=xhf&password=xiehaifeng&oldpassword=xiehaifeng）*/
...
//do something ···start
	...
	connection.query(sql,function (err, result) {
	        ...
	        console.log('--------------------------SELECT  start----------------------------');
	        console.log("开始查询----%O",JSON.parse(JSON.stringify(result)));
	        console.log("请求参数----用户名（%s）----新密码（%s）---原密码（%s）",req.query.usename,req.query.password,req.query.oldpassword);
	        //查询出来的是一个数组对象所以直接用JSON.parse(JSON.stringify(result))。password会出现undefined！！！
	        if(JSON.parse(JSON.stringify(result)).length!=0){
	        	//改 --mysql命令
				...
				if (result[0].password == req.query.oldpassword) {					
	...
	//do something -- 数据库操作结束	
	console.log(">>>>>>>>一次新的/resetpassword请求>>>>>>>>");    
});
